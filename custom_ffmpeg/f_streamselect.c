...
//je declare le fichier a ouvrir ici
FILE *fichier;
int selected_stream;
int entier;
//**********************************
...
typedef struct StreamSelectContext {
    ...
    int *map;
    int nb_map;
    int is_audio;		///determine si les streams sont audios (le int equivaut a un booleen) 
    int64_t *last_pts;
    AVFrame **frames;		///on a un pointeur vers un ensemble de pointeurs vers des AVFrames, c'est la que sont selectionnes les differents streams
    FFFrameSync fs;
} StreamSelectContext;
...
static int process_frame(FFFrameSync *fs)
{
    ...
    for (i = 0; i < ctx->nb_inputs; i++) {		///i parcours les differents streams disponibles
        if ((ret = ff_framesync_get_frame(&s->fs, i, &in[i], 0)) < 0)  ///la variable 'in' stocke la frame du stream i et est initialise avec s->frames qui est un pointeur de pointeur vers une AVFrame
            return ret;			///i est l'index de l'input
    }
	
	//********************************************************************
	//Modification du code : ouverture du fichier et lecture
	fichier = fopen("./src/streamingMultimedia/streamInit/SelectStream.txt", "r");
	
	if(fichier == NULL){
	printf("pas de fichier de SelectStream\n");
	}else{
		entier = fscanf(fichier, "%d" ,&selected_stream);
		printf("selected stream = %d \n", selected_stream);
		if(selected_stream < 0 || selected_stream > 2 ){ ///on verifie la coherence des donnees en evitant des valeurs delirantes
			selected_stream = 0;
			printf("mauvaise valeur assignee a selected_stream");
		}
		fclose(fichier);
	}
	//*******************************************************************
    ...
    for (j = 0; j < ctx->nb_inputs; j++) {
        for (i = 0; i < s->nb_map; i++) {
            //if (s->map[i] == j) { 		//ancien code
            if (selected_stream == j) {		//nouveau code
                AVFrame *out;

                if (s->is_audio && s->last_pts[j] == in[j]->pts &&
                    ctx->outputs[i]->frame_count_in > 0)
                    continue;
                out = av_frame_clone(in[j]);			///on choisit le stream a selectionner ici
                if (!out)
...
static av_cold int init(AVFilterContext *ctx)
{
    ...
    if (!strcmp(ctx->filter->name, "astreamselect"))	///on verifie si les streams sont audio ici en comparant avec strcmp (string compare) le nom du filtre et astreamselect
        s->is_audio = 1;
