...
static inline int normalize_double(int *n, double d)
{
    ...
    if (isnan(d)) {
        ...
        *n = lrint(d); //en gros ici si le double est correct vis a vis des dimension de l image, on fait appel a lrint qui 'Round and cast to long integer' 
...
static int config_input(AVFilterLink *link)
{
    ...
    s->var_values[VAR_IN_W]  = s->var_values[VAR_IW] = ctx->inputs[0]->w; //essayer de modifier ici pour avoir le w et le h depuis l'exterieur
...
static int filter_frame(AVFilterLink *link, AVFrame *frame)
{
	...
	// CUSTOM MODIFICATION: Open file , read the crop window coord and close it 
	// The point is to get the coords from a text file written by another program
	// The coord are determined by the motion estimation / tracking program and written to the hardcoded path there
	
	// it's really bad for performance but for just prototype purpose this will do fine
	
	fichier = fopen("./src/streamingMultimedia/streamInit/crop.txt", "r");
	if(fichier == NULL){
	printf("pas de fichier de crop\n");
	s->var_values[VAR_X] = av_expr_eval(s->x_pexpr, s->var_values, NULL);
    s->var_values[VAR_Y] = av_expr_eval(s->y_pexpr, s->var_values, NULL);
    s->var_values[VAR_X] = av_expr_eval(s->x_pexpr, s->var_values, NULL);
	}else{
		entier = fscanf(fichier, "%d %d" ,&input_x, &input_y);
		s->var_values[VAR_X] = (double)input_x;
		s->var_values[VAR_Y] = (double)input_y;
		fclose(fichier);
	}
	...
	//s->var_values[VAR_X] = av_expr_eval(s->x_pexpr, s->var_values, NULL);// il semblerait que l expression de x et y soit evalue a chaque appel
    //s->var_values[VAR_Y] = av_expr_eval(s->y_pexpr, s->var_values, NULL);//si on modifie il faut que l on renvoie un double dans var_values[VAR_Y]
    //s->var_values[VAR_X] = av_expr_eval(s->x_pexpr, s->var_values, NULL);
