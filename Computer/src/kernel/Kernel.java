...
public class Kernel implements 	IhmInterface, 
	...
	public void initModules()
	{
		...
		if (Constants.MOBILE_CAM_CONTROL_ON) {
			Orientation topLeft = new Orientation(95,116);
			Orientation topRight = new Orientation(79,116);
			Orientation bottomLeft = new Orientation(95,132);
			Orientation bottomRight = new Orientation(79,132);		
			camera.initLocator(topLeft, topRight, bottomLeft, bottomRight);
			camera.initCamera();
			try {
				camera.setAim(new Point(0,0));
	...
	public void startThreads()
	{
		...
		try {
			zones.get(0).setCenter(new Point(100,100));
			zones.get(1).setCenter(new Point(1600,800));
		} catch (OutofFrameException e) {
			e.printStackTrace();
		}
	}
	...
	public void zoomPlus() {
		Log.logger.info("IHM : Zoom avant");
		camera.setZoom(camera.getZoom()+1);		
	}

	@Override
	public void zoomMinus() {
		Log.logger.info("IHM : Zoom arriere");
		camera.setZoom(camera.getZoom()-1);		
